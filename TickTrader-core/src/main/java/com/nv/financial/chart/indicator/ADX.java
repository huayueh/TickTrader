...
/**
 * Average Directional Movement Index. Base on
...
public class ADX extends AbstractIndicator {
    private static final Logger logger = LogManager.getLogger(ADX.class);
	...
	public ADX(IQuoteProvider quoteProvider, int periodLength) {
        super(quoteProvider);
...
//        this.k = (int)Math.ceil(3.45 * (periodLength + 1));
    ...
    public IndicatorValue calculate(long time, QuotePrice quotePrice) {
        ...
        RetCode retCode = talib.adx(0, size-1, high, low, close, periodLength, outBegin, outNBElement, out);
